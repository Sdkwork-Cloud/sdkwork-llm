...
const openApiPrefix = "/llm/v3/api";
const openApiSchemaUrl = "/llm/v3/openapi.json";
...
const openAuthority = rootSpec.contracts.apiAuthorities.find(
  (authority) => authority.name === "sdkwork-llm-open-api",
);
assert.ok(openAuthority, "Root component spec must declare sdkwork-llm-open-api");
assert.equal(
...
assert.equal(
  ...
  openApiPrefix,
  "Memory open SDK assembly must use the /llm/v3/api public prefix",
);
assert.equal(
  assembly.discoverySurface.schemaUrl,
  openApiSchemaUrl,
  "Memory open SDK assembly must expose the /llm/v3/openapi.json schema URL",
);
assert.notEqual(
  assembly.discoverySurface.schemaUrl,
...
const sdkManifest = readJson("sdks/sdkwork-llm-sdk/sdk-manifest.json");
assert.equal(
  sdkManifest.apiPrefix,
  openApiPrefix,
  "Memory open SDK manifest must use the /llm/v3/api public prefix",
);

const sdkComponent = readJson("sdks/sdkwork-llm-sdk/specs/component.spec.json");
assert.equal(
  sdkComponent.contracts.apiAuthority.prefix,
  openApiPrefix,
  "Memory open SDK component spec must use the /llm/v3/api public prefix",
);

const openApi = readJson("sdks/sdkwork-llm-sdk/openapi/llm-open-api.openapi.json");
assert.equal(
  openApi["x-sdkwork-api-prefix"],
  openApiPrefix,
  "Memory open OpenAPI authority must advertise the /llm/v3/api public prefix",
);
