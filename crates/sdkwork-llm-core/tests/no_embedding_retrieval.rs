use sdkwork_llm_contract::{LlmRecord, LlmRecordType};
use sdkwork_llm_core::{fuse_retrieval_candidates, keyword_match_score, RetrievalCandidate};

fn sample_record(id: u64, text: &str) -> LlmRecord {
    LlmRecord {
        record_id: id,
...
fn fuse_retrieval_candidates_ranks_by_score_without_embeddings() {
    ...
    assert_eq!(fused[0].memory.record_id, 2);
