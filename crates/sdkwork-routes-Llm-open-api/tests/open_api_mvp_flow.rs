...
use sdkwork_routes_llm_open_api::build_router_with_open_api;
use serde_json::json;
...
async fn open_api_mvp_flow_memory_retrieval_and_context_pack_without_embeddings() {
    ...
    let context_pack = app
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/llm/v3/api/llm/context_packs")
                .header("content-type", "application/json")
                .extension(open_context())
                .body(Body::from(
                    json!({
...
async fn open_api_mvp_flow_event_extraction_candidates_and_feedback() {
    ...
    let extraction = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/llm/v3/api/llm/extractions")
                .header("content-type", "application/json")
                .extension(open_context())
                .body(Body::from(
                    json!({
    ...
    let candidates = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/llm/v3/api/llm/candidates?spaceId=1")
                .extension(open_context())
                .body(Body::empty())
                .unwrap(),
        )
    ...
    let candidate = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(format!("/llm/v3/api/llm/candidates/{candidate_id}"))
                .extension(open_context())
                .body(Body::empty())
                .unwrap(),
        )
