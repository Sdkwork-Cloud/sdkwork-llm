...
impl OpenLlmService {
    ...
    fn memory_index_json(row: &NativeSqlMemoryIndexRow) -> LlmServiceResult<serde_json::Value> {
        let config = row
            .config_json
            .as_deref()
    ...
    fn retrieval_profile_json(
        ...
        let retrievers: serde_json::Value =
            serde_json::from_str(&row.retrievers_json).map_err(|error| {
                LlmServiceError::storage(format!(
    ...
    fn eval_run_json(row: &NativeSqlEvalRunRow) -> LlmServiceResult<serde_json::Value> {
        let metrics = row
            .metrics_json
            .as_deref()
            .and_then(|value| serde_json::from_str(value).ok())
    ...
    async fn save_governance_entity(
        ...
        let metadata = serde_json::to_string(entity).map_err(|error| {
            LlmServiceError::storage(format!("governance entity encode failed: {error}"))
        })?;
        self.store
    ...
    async fn load_governance_entity(
        ...
        let metadata = self
            ...
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::not_found("governance entity not found"))?;
        serde_json::from_str(&metadata).map_err(|error| {
            LlmServiceError::storage(format!("governance entity decode failed: {error}"))
        })
    }
    ...
    pub(crate) async fn backend_list_indexes(
        ...
        let rows = self
            .store
            .list_llm_indexes_for_tenant(tenant_id, page_size)
            .await
            .map_err(OpenLlmService::map_store_error)?;
        let items = rows
            .iter()
    ...
    pub(crate) async fn backend_create_index(
        ...
        let config_json = request
            ...
            .transpose()
            .map_err(|error| {
                LlmServiceError::storage(format!("index config encode failed: {error}"))
            })?;
        self.store
            .insert_llm_index(
                tenant_id,
        ...
        let row = self
            .store
            .retrieve_llm_index_for_tenant(tenant_id, &index_id)
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::storage("created index could not be loaded"))?;
    ...
    pub(crate) async fn backend_retrieve_index(
        ...
        let row = self
            .store
            .retrieve_llm_index_for_tenant(tenant_id, &index_id.to_string())
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::not_found("memory index not found"))?;
        Self::memory_index_json(&row)
    }
    ...
    pub(crate) async fn backend_list_retrieval_profiles(
        ...
        let rows = self
            .store
            .list_llm_retrieval_profiles_for_tenant(tenant_id, page_size)
            .await
            .map_err(OpenLlmService::map_store_error)?;
        let items = rows
            .iter()
    ...
    pub(crate) async fn backend_create_retrieval_profile(
        ...
        let retrievers_json = request
            ...
            .transpose()
            .map_err(|error| {
                LlmServiceError::storage(format!("retrievers encode failed: {error}"))
            })?
        ...
        let row = self
            .store
            .retrieve_llm_retrieval_profile_for_tenant(tenant_id, &profile_id)
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::storage("created profile could not be loaded"))?;
    ...
    pub(crate) async fn backend_retrieve_retrieval_profile(
        ...
        let row = self
            .store
            .retrieve_llm_retrieval_profile_for_tenant(tenant_id, &profile_id.to_string())
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::not_found("retrieval profile not found"))?;
        Self::retrieval_profile_json(&row)
    }
    ...
    pub(crate) async fn backend_update_retrieval_profile(
        ...
        let row = self
            .store
            .update_llm_retrieval_profile_for_tenant(
                tenant_id,
    ...
    pub(crate) async fn backend_list_implementation_profiles(
        ...
        let rows = self
            .store
            .list_llm_implementation_profiles_for_tenant(tenant_id, page_size)
            .await
            .map_err(OpenLlmService::map_store_error)?;
        let items = rows
            .iter()
    ...
    pub(crate) async fn backend_create_implementation_profile(
        ...
        let capability_json = request
            ...
            .transpose()
            .map_err(|error| {
                LlmServiceError::storage(format!("capabilities encode failed: {error}"))
            })?
        ...
        let row = self
            .store
            .retrieve_llm_implementation_profile_for_tenant(tenant_id, &profile_id)
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::storage("created profile could not be loaded"))?;
    ...
    pub(crate) async fn backend_retrieve_implementation_profile(
        ...
        let row = self
            .store
            .retrieve_llm_implementation_profile_for_tenant(tenant_id, &profile_id.to_string())
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::not_found("implementation profile not found"))?;
        Self::implementation_profile_json(&row)
    }
    ...
    pub(crate) async fn backend_update_implementation_profile(
        ...
        let row = self
            .store
            .update_llm_implementation_profile_for_tenant(
                tenant_id,
                &profile_id.to_string(),
                request.get("name").and_then(|value| value.as_str()),
                request
    ...
    pub(crate) async fn backend_list_provider_bindings(
        ...
        let rows = self
            .store
            .list_llm_provider_bindings_for_tenant(tenant_id, page_size)
            .await
            .map_err(OpenLlmService::map_store_error)?;
        Ok(serde_json::json!({
    ...
    pub(crate) async fn backend_create_provider_binding(
        ...
        let row = self
            .store
            .retrieve_llm_provider_binding_for_tenant(tenant_id, &binding_id)
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::storage("created binding could not be loaded"))?;
    ...
    pub(crate) async fn backend_update_provider_binding(
        ...
        let row = self
            .store
            .update_llm_provider_binding_for_tenant(
                tenant_id,
            ...
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::not_found("provider binding not found"))?;
        Ok(Self::provider_binding_json(&row))
    }
    ...
    pub(crate) async fn backend_list_eval_runs(
        ...
        let rows = self
            .store
            .list_llm_eval_runs_for_tenant(tenant_id, page_size)
            .await
            .map_err(OpenLlmService::map_store_error)?;
        let items = rows
            .iter()
    ...
    pub(crate) async fn backend_create_eval_run(
        ...
        let metrics_json = request
            ...
            .transpose()
            .map_err(|error| {
                LlmServiceError::storage(format!("eval metrics encode failed: {error}"))
            })?;
        self.store
            .insert_llm_eval_run(
                tenant_id,
        ...
        let row = self
            .store
            .retrieve_llm_eval_run_for_tenant(tenant_id, &eval_run_id)
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::storage("created eval run could not be loaded"))?;
    ...
    pub(crate) async fn backend_retrieve_eval_run(
        ...
        let row = self
            .store
            .retrieve_llm_eval_run_for_tenant(tenant_id, &eval_run_id.to_string())
            .await
            .map_err(OpenLlmService::map_store_error)?
            .ok_or_else(|| LlmServiceError::not_found("eval run not found"))?;
        Self::eval_run_json(&row)
    }
    ...
    pub(crate) async fn backend_create_extraction_job(
        ...
        let input_events = request
            ...
            .unwrap_or_default();
        let job = LlmOpenApi::create_extraction(
            self,
        ...
        let entity = serde_json::to_value(&job).map_err(|error| {
            LlmServiceError::storage(format!("extraction job encode failed: {error}"))
        })?;
        let tenant_id = i64::try_from(context.tenant_id).unwrap_or(i64::MAX);
    ...
    pub(crate) async fn backend_supersede_memory(
        ...
        let canonical_text = request
            .get("canonicalText")
            .and_then(|value| value.as_str())
            .ok_or_else(|| LlmServiceError::validation("canonicalText is required"))?;
