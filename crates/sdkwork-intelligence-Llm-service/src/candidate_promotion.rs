use sdkwork_llm_contract::{LlmCandidate, LlmServiceError, LlmServiceResult};
use sdkwork_llm_plugin_native_sql::NativeSqlCandidateDetailRow;
use sdkwork_llm_spi::{ApproveLlmCandidateCommand, LlmScopeContext};
...
impl OpenLlmService {
    pub(crate) async fn approve_candidate_with_promotion(
        ...
        if refreshed.target_memory_uuid.as_deref() != Some(memory_uuid.as_str()) {
            return Err(LlmServiceError::storage(
