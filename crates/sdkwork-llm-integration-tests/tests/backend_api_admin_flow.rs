...
async fn backend_api_indexes_and_retrieval_profiles_return_phase1_defaults() {
    ...
    let indexes = app
        .clone()
        .oneshot(authed_get("/backend/v3/api/llm/indexes"))
        .await
        .unwrap();
    ...
    let profiles = app
        .oneshot(authed_get("/backend/v3/api/llm/retrieval_profiles"))
        .await
        .unwrap();
...
async fn backend_api_migration_job_round_trip_via_dual_token() {
    ...
    let retrieve = app
        .oneshot(authed_get(&format!(
            "/backend/v3/api/llm/migration_jobs/{job_id}"
        )))
        .await
        .unwrap();
...
async fn backend_api_admin_config_persists_in_sql_tables() {
    ...
    assert!(
        ...
        "table-backed admin entities must not use llm_audit_log admin.config.save"
    );
}
