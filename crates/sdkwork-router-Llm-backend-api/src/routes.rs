...
pub fn build_router_with_shared_backend_api(api: Arc<dyn LlmBackendApi>) -> Router {
    Router::new()
        .route(paths::SPACES, get(list_spaces))
        .route(paths::SPACE, get(retrieve_space).patch(update_space))
        .route(paths::RECORDS, get(list_memories))
        .route(paths::RECORD, get(retrieve_memory).patch(update_memory))
        .route(paths::RECORD_SUPERSEDE, post(supersede_memory))
...
async fn supersede_memory(
    ...
    ok_json(
        state
            .api
            .supersede_memory(context, record_id, request)
            .await,
    )
}
...
fn ok_json<T>(result: LlmServiceResult<T>) -> Result<Response, BackendApiProblem>
where
    T: serde::Serialize,
{
    match result {
...
fn created_json<T>(result: LlmServiceResult<T>) -> Result<Response, BackendApiProblem>
where
    T: serde::Serialize,
{
    match result {
