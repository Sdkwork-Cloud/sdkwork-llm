...
pub fn llm_app_api_public_path_prefixes() -> Vec<String> {
    vec![paths::HEALTHZ.to_owned()]
}

pub fn llm_app_api_prefixes() -> Vec<String> {
    vec![paths::PREFIX.to_owned()]
}

#[derive(Clone, Default)]
struct LlmAppContextInjector;

impl DomainContextInjector for LlmAppContextInjector {
    fn inject(&self, request: &mut axum::extract::Request, context: &WebRequestContext) {
        if let Some(app_context) = llm_app_context_from_web_request(context) {
...
pub fn wrap_router_with_web_framework(
    ...
    with_web_request_context(router, build_llm_app_api_framework_layer(resolver))
}
...
fn build_llm_app_api_framework_layer<R>(resolver: R) -> WebFrameworkLayer<R>
where
    R: sdkwork_web_core::WebRequestContextResolver + Clone,
{
    let route_manifest = app_route_manifest();
    route_manifest
        .validate_public_path_prefixes(&llm_app_api_public_path_prefixes())
    ...
    WebFrameworkLayer::new(resolver)
        .with_profile(WebRequestContextProfile {
            ...
            public_path_prefixes: llm_app_api_public_path_prefixes(),
            ..WebRequestContextProfile::default()
        })
        .with_route_manifest(route_manifest)
        .with_domain_injector(Arc::new(LlmAppContextInjector))
}
...
pub async fn wrap_router_with_web_framework_from_env(router: Router) -> Router {
    match llm_web_auth_mode_from_env().await {
        ...
        LlmWebAuthMode::ProductionFailClosed => with_web_request_context(
            router,
            build_llm_app_api_framework_layer(ProductionFailClosedResolver),
        ),
