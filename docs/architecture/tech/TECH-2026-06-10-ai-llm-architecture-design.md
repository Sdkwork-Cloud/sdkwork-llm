> Migrated from `docs/superpowers/specs/2026-06-10-ai-llm-architecture-design.md` on 2026-06-24.
...
This document defines a complete, evolvable architecture for an AI LLM memory module. The module must support self-learning memory, habit-forming memory, multi-scope long-term memory, provider-agnostic model integration, and retrieval without requiring embeddings.
...
```ts
export interface LlmCandidateHit {
  memoryId: string;
  retriever: string;
  matchType:
    | "exact"
...
5. Confirmed user memory beats inferred habit LLM memory.
6. Tenant policy beats user preference.
7. Current user instruction beats historical habit LLM memory for the current turn.
...
Project state:
- This project is designing an embedding-optional, multi-index AI LLM memory service. source=mem_456 confidence=0.88
...
POST   /v1/records/forget
POST   /v1/records/extract
POST   /v1/records/consolidate
```
...
```text
GET  /v1/audit-logs
GET  /v1/memory-sources/{recordId}
POST /v1/export
...
```text
Application/repository: sdkwork-llm
Domain: intelligence
Capability: memory
...
App API:
  Prefix: /app/v3/api
  Authority metadata: sdkwork-llm.app
  OpenAPI file: sdks/sdkwork-llm-app-sdk/openapi/llm-app-api.openapi.json
  SDK family: sdks/sdkwork-llm-app-sdk

Backend API:
  Prefix: /backend/v3/api
  Authority metadata: sdkwork-llm.backend
  OpenAPI file: sdks/sdkwork-llm-backend-sdk/openapi/llm-backend-api.openapi.json
  SDK family: sdks/sdkwork-llm-backend-sdk

Open API:
  Prefix: /llm/v3/api
  Authority metadata: sdkwork-llm-open-api
  OpenAPI file: sdks/sdkwork-llm-sdk/openapi/llm-open-api.openapi.json
  SDK family: sdks/sdkwork-llm-sdk
  Credential mode: X-API-Key / ApiKey

RPC SDK:
  Optional later as sdkwork-llm-rpc-sdk for backend-to-backend or local/private high-throughput runtimes.
```
...
sdkwork-llm/
  ...
  docs/
    schema-registry/
      tables/
        001-llm-core.yaml
        002-llm-learning.yaml
        003-llm-retrieval.yaml
        004-llm-provider.yaml
        005-llm-governance.yaml
    superpowers/
      specs/
      plans/
  crates/
    ...
    sdkwork-llm-core/
    sdkwork-llm-agent-provider/
    sdkwork-llm-drive/
    sdkwork-llm-test-support/
  packages/
    native-rust/
      routes/
        open-api/
  ...
  sdks/
    sdkwork-llm-sdk/
      ...
      openapi/
        llm-open-api.openapi.json
        llm-open-api.sdkgen.yaml
      sdkwork-llm-sdk-typescript/
    ...
    sdkwork-llm-app-sdk/
      ...
      specs/component.spec.json
      openapi/
        llm-app-api.openapi.json
        llm-app-api.sdkgen.yaml
      sdkwork-llm-app-sdk-typescript/
        generated/server-openapi/
        composed/
    sdkwork-llm-backend-sdk/
      ...
      specs/component.spec.json
      openapi/
        llm-backend-api.openapi.json
        llm-backend-api.sdkgen.yaml
      sdkwork-llm-backend-sdk-typescript/
        generated/server-openapi/
        composed/
  tools/
...
| --- | --- |
| `sdkwork-llm-contract` | Public DTOs, enums, operation IDs, API schema helpers, provider manifest contracts. |
| `sdkwork-llm-core` | ID helpers, scoring utilities, lifecycle state machines, query planning helpers. |
| `sdkwork-llm-product` | Use cases, policy engine, learning engine, consolidation engine, retrieval orchestration, context assembly. |
| `sdkwork-llm-storage-sqlx` | PostgreSQL/SQLite migrations, repository implementations, schema migration registry. |
| `sdkwork-routes-llm-open-api` | Open API path constants, route manifest, router registration, and handler boundary for `/llm/v3/api`. |
| `sdkwork-routes-llm-app-api` | App API path constants, route manifest, router registration, and handler boundary for `/app/v3/api`. |
| `sdkwork-routes-llm-backend-api` | Backend API path constants, route manifest, router registration, and handler boundary for `/backend/v3/api`. |
| `sdkwork-llm-open-api` | Open HTTP route boundary for external and server-to-server API-key Memory integrations. |
| `sdkwork-llm-app-api` | App HTTP route boundary for user-facing and agent-facing generated App SDK operations. |
| `sdkwork-llm-backend-api` | Backend HTTP route boundary for `backend-admin`, operator, and control-plane generated Backend SDK operations. |
| `sdkwork-llm-agent-provider` | Adapter from Memory context/retrieval contracts to agent runtimes. |
| `sdkwork-llm-drive` | Adapter for Drive-backed exports, imports, snapshots, replay packages, and large event payload references. |
| `sdkwork-llm-test-support` | Fake stores, fake providers, deterministic event fixtures, eval datasets. |
...
4. External provider bridges must keep local `llm_record`, `llm_event`, and `llm_audit_log` shadow records sufficient for source tracing, deletion, export, and SDKWork governance.
...
```ts
export interface LlmRecordStore {
  create(command: CreateLlmRecordCommand): Promise<LlmRecord>;
  retrieve(scope: LlmScopeContext, memoryId: string): Promise<LlmRecord | null>;
  list(query: LlmRecordQuery): Promise<Page<LlmRecord>>;
  update(command: UpdateLlmRecordCommand): Promise<LlmRecord>;
  markDeleted(command: DeleteLlmRecordCommand): Promise<void>;
}

export interface LlmEventStore {
  append(command: AppendLlmEventCommand): Promise<LlmEvent>;
  retrieve(eventId: string): Promise<LlmEvent | null>;
  list(query: LlmEventQuery): Promise<Page<LlmEvent>>;
}

export interface LlmCandidateStore {
  create(candidate: LlmCandidate): Promise<LlmCandidate>;
  decide(command: DecideLlmCandidateCommand): Promise<MemoryDecisionResult>;
  list(query: LlmCandidateQuery): Promise<Page<LlmCandidate>>;
}
...
```text
sdks/sdkwork-llm-rpc-sdk   # RPC, only if backend/native high-throughput integration is needed
```
...
```json
{
  "workspace": "sdkwork-llm-sdk",
  "title": "SDKWork LLM Open API SDK",
  "apiVersion": "0.1.0",
  "openapiVersion": "3.1.2",
  "authoritySpec": "openapi/llm-open-api.openapi.json",
  "generationInputSpec": "openapi/llm-open-api.openapi.json",
  "apiAuthority": "sdkwork-llm-open-api",
  "discoverySurface": {
    "sdkTarget": "open-api",
    "apiPrefix": "/llm/v3/api",
    "schemaUrl": "/llm/v3/openapi.json",
    "generatedProtocols": ["http-openapi"],
    "manualTransports": []
  },
  "sdkOwner": "sdkwork-llm",
  "sdkDependencies": []
}
```
...
```json
{
  "workspace": "sdkwork-llm-app-sdk",
  "title": "SDKWork LLM App API SDK",
  "apiVersion": "0.1.0",
  "openapiVersion": "3.1.2",
  "authoritySpec": "openapi/llm-app-api.openapi.json",
  "generationInputSpec": "openapi/llm-app-api.openapi.json",
...
```json
{
  "workspace": "sdkwork-llm-backend-sdk",
  "title": "SDKWork LLM Backend API SDK",
  "apiVersion": "0.1.0",
  "openapiVersion": "3.1.2",
  "authoritySpec": "openapi/llm-backend-api.openapi.json",
  "generationInputSpec": "openapi/llm-backend-api.openapi.json",
...
All open-api paths use `/llm/v3/api`. Protected operations require the Open API credential mode `ApiKey` through the `X-API-Key` header. DTO fields containing `int64` values must serialize as strings.
...
| --- | --- | --- | --- |
| `GET` | `/llm/v3/api/llm/capabilities` | `capabilities.retrieve` | Retrieve supported memory capabilities and implementation-safe feature flags. |
| `POST` | `/llm/v3/api/llm/events` | `events.create` | Append external evidence event through API-key context. |
| `GET` | `/llm/v3/api/llm/events/{eventId}` | `events.retrieve` | Retrieve an allowed external event projection. |
| `GET` | `/llm/v3/api/llm/records` | `records.list` | List/search memory records allowed to the API key. |
| `POST` | `/llm/v3/api/llm/records` | `records.create` | Explicitly create llm record from external integration. |
| `GET` | `/llm/v3/api/llm/records/{recordId}` | `records.retrieve` | Retrieve a memory record. |
| `PATCH` | `/llm/v3/api/llm/records/{recordId}` | `records.update` | Update a memory record without exposing backend-only controls. |
| `DELETE` | `/llm/v3/api/llm/records/{recordId}` | `records.delete` | Delete/suppress a memory record through scoped API-key authority. |
| `POST` | `/llm/v3/api/llm/retrievals` | `retrievals.create` | Retrieve memory using a server-approved profile. |
| `GET` | `/llm/v3/api/llm/retrievals/{retrievalId}` | `retrievals.retrieve` | Retrieve retrieval trace projection allowed to the API key. |
| `POST` | `/llm/v3/api/llm/context_packs` | `contextPacks.create` | Assemble model-ready memory context. |
| `GET` | `/llm/v3/api/llm/context_packs/{contextPackId}` | `contextPacks.retrieve` | Retrieve assembled context pack metadata. |
| `POST` | `/llm/v3/api/llm/feedback` | `feedback.create` | Submit usefulness, correction, or suppression feedback. |
| `POST` | `/llm/v3/api/llm/extractions` | `extractions.create` | Extract candidate memories from supplied external events/messages. |
| `GET` | `/llm/v3/api/llm/candidates` | `candidates.list` | List API-key-visible candidate memories. |
| `GET` | `/llm/v3/api/llm/candidates/{candidateId}` | `candidates.retrieve` | Retrieve a candidate memory. |
| `GET` | `/llm/v3/api/llm/provider_health` | `providerHealth.retrieve` | Retrieve a redacted provider health summary. |

Open API scopes:

```text
llm.capabilities.read
llm.events.write
llm.events.read
llm.records.read
llm.records.write
llm.records.delete
llm.retrievals.create
llm.contextPacks.create
llm.feedback.write
...
| Method | Path | operationId | Purpose |
| --- | --- | --- | --- |
| `POST` | `/app/v3/api/llm/spaces` | `spaces.create` | Create user/project memory space. |
| `GET` | `/app/v3/api/llm/spaces` | `spaces.list` | List accessible memory spaces. |
| `GET` | `/app/v3/api/llm/spaces/{spaceId}` | `spaces.retrieve` | Retrieve memory space. |
| `PATCH` | `/app/v3/api/llm/spaces/{spaceId}` | `spaces.update` | Update memory space settings. |
| `POST` | `/app/v3/api/llm/events` | `events.create` | Append raw memory evidence event. |
| `GET` | `/app/v3/api/llm/events/{eventId}` | `events.retrieve` | Retrieve safe event metadata/content. |
| `POST` | `/app/v3/api/llm/records` | `records.create` | Explicitly create llm record. |
| `GET` | `/app/v3/api/llm/records` | `records.list` | List/search memory records. |
| `GET` | `/app/v3/api/llm/records/{recordId}` | `records.retrieve` | Retrieve memory record. |
| `PATCH` | `/app/v3/api/llm/records/{recordId}` | `records.update` | Update memory record. |
| `DELETE` | `/app/v3/api/llm/records/{recordId}` | `records.delete` | Delete/suppress memory record. |
| `GET` | `/app/v3/api/llm/records/{recordId}/sources` | `records.sources.list` | Explain memory source events. |
| `POST` | `/app/v3/api/llm/forget_requests` | `forgetRequests.create` | Request scoped memory deletion/forgetting. |
| `GET` | `/app/v3/api/llm/forget_requests/{forgetRequestId}` | `forgetRequests.retrieve` | Retrieve forget workflow status. |
| `POST` | `/app/v3/api/llm/extractions` | `extractions.create` | Extract candidate memories from supplied events/messages. |
| `GET` | `/app/v3/api/llm/candidates` | `candidates.list` | List user-reviewable memory candidates. |
| `GET` | `/app/v3/api/llm/candidates/{candidateId}` | `candidates.retrieve` | Retrieve memory candidate. |
| `POST` | `/app/v3/api/llm/candidates/{candidateId}/approve` | `candidates.approve` | Approve candidate memory. |
| `POST` | `/app/v3/api/llm/candidates/{candidateId}/reject` | `candidates.reject` | Reject candidate memory. |
| `GET` | `/app/v3/api/llm/habits` | `habits.list` | List habit memories. |
| `GET` | `/app/v3/api/llm/habits/{habitId}` | `habits.retrieve` | Retrieve habit LLM memory. |
| `PATCH` | `/app/v3/api/llm/habits/{habitId}` | `habits.update` | Update habit activation/settings. |
| `POST` | `/app/v3/api/llm/habits/{habitId}/confirm` | `habits.confirm` | Confirm inferred habit. |
| `POST` | `/app/v3/api/llm/habits/{habitId}/reject` | `habits.reject` | Reject inferred habit. |
| `POST` | `/app/v3/api/llm/retrievals` | `retrievals.create` | Retrieve memory using configured profile. |
| `GET` | `/app/v3/api/llm/retrievals/{retrievalId}` | `retrievals.retrieve` | Retrieve retrieval trace allowed to user. |
| `POST` | `/app/v3/api/llm/context_packs` | `contextPacks.create` | Assemble model-ready memory context. |
| `GET` | `/app/v3/api/llm/context_packs/{contextPackId}` | `contextPacks.retrieve` | Retrieve assembled context pack metadata. |
| `POST` | `/app/v3/api/llm/feedback` | `feedback.create` | Submit usefulness/correction feedback. |
| `POST` | `/app/v3/api/llm/export_jobs` | `exportJobs.create` | Request user/space memory export. |
| `GET` | `/app/v3/api/llm/export_jobs/{exportJobId}` | `exportJobs.retrieve` | Retrieve export job status and Drive ref. |
| `GET` | `/app/v3/api/llm/learning_settings` | `learningSettings.retrieve` | Retrieve user learning settings. |
| `PATCH` | `/app/v3/api/llm/learning_settings` | `learningSettings.update` | Enable/disable learning and habit inference. |
...
| Method | Path | operationId | Purpose |
| --- | --- | --- | --- |
| `GET` | `/backend/v3/api/llm/spaces` | `spaces.list` | Admin list memory spaces. |
| `GET` | `/backend/v3/api/llm/spaces/{spaceId}` | `spaces.retrieve` | Admin retrieve memory space. |
| `PATCH` | `/backend/v3/api/llm/spaces/{spaceId}` | `spaces.update` | Admin update memory space state/policy. |
| `GET` | `/backend/v3/api/llm/records` | `records.list` | Admin search memory records. |
| `GET` | `/backend/v3/api/llm/records/{recordId}` | `records.retrieve` | Admin retrieve memory record. |
| `PATCH` | `/backend/v3/api/llm/records/{recordId}` | `records.update` | Admin correct/suppress memory record. |
| `POST` | `/backend/v3/api/llm/records/{recordId}/supersede` | `memories.supersede` | Supersede memory with corrected fact. |
| `GET` | `/backend/v3/api/llm/events` | `events.list` | Admin list evidence events. |
| `GET` | `/backend/v3/api/llm/events/{eventId}` | `events.retrieve` | Admin retrieve event details. |
| `GET` | `/backend/v3/api/llm/candidates` | `candidates.list` | Admin list candidate memories. |
| `POST` | `/backend/v3/api/llm/candidates/{candidateId}/approve` | `candidates.approve` | Admin approve candidate. |
| `POST` | `/backend/v3/api/llm/candidates/{candidateId}/reject` | `candidates.reject` | Admin reject candidate. |
| `POST` | `/backend/v3/api/llm/extraction_jobs` | `extractionJobs.create` | Run extraction over events. |
| `GET` | `/backend/v3/api/llm/extraction_jobs/{jobId}` | `extractionJobs.retrieve` | Retrieve extraction job. |
| `POST` | `/backend/v3/api/llm/consolidation_jobs` | `consolidationJobs.create` | Run dedupe/merge/supersession. |
| `POST` | `/backend/v3/api/llm/indexes` | `indexes.create` | Create index definition. |
| `GET` | `/backend/v3/api/llm/indexes` | `indexes.list` | List index definitions. |
| `GET` | `/backend/v3/api/llm/indexes/{indexId}` | `indexes.retrieve` | Retrieve index definition. |
| `PATCH` | `/backend/v3/api/llm/indexes/{indexId}` | `indexes.update` | Update index definition. |
| `POST` | `/backend/v3/api/llm/indexes/{indexId}/rebuild` | `indexes.rebuild` | Rebuild derived index. |
| `POST` | `/backend/v3/api/llm/retrieval_profiles` | `retrievalProfiles.create` | Create retrieval profile. |
| `GET` | `/backend/v3/api/llm/retrieval_profiles` | `retrievalProfiles.list` | List retrieval profiles. |
| `GET` | `/backend/v3/api/llm/retrieval_profiles/{profileId}` | `retrievalProfiles.retrieve` | Retrieve retrieval profile. |
| `PATCH` | `/backend/v3/api/llm/retrieval_profiles/{profileId}` | `retrievalProfiles.update` | Update retrieval profile. |
| `POST` | `/backend/v3/api/llm/implementation_profiles` | `implementationProfiles.create` | Create implementation profile. |
| `GET` | `/backend/v3/api/llm/implementation_profiles` | `implementationProfiles.list` | List implementation profiles. |
| `GET` | `/backend/v3/api/llm/implementation_profiles/{profileId}` | `implementationProfiles.retrieve` | Retrieve implementation profile. |
| `PATCH` | `/backend/v3/api/llm/implementation_profiles/{profileId}` | `implementationProfiles.update` | Update implementation profile. |
| `POST` | `/backend/v3/api/llm/provider_bindings` | `providerBindings.create` | Register provider binding. |
| `GET` | `/backend/v3/api/llm/provider_bindings` | `providerBindings.list` | List provider bindings. |
| `PATCH` | `/backend/v3/api/llm/provider_bindings/{bindingId}` | `providerBindings.update` | Update provider binding. |
| `GET` | `/backend/v3/api/llm/provider_health` | `providerHealth.retrieve` | Retrieve provider health summary. |
| `POST` | `/backend/v3/api/llm/eval_runs` | `evalRuns.create` | Start memory eval run. |
| `GET` | `/backend/v3/api/llm/eval_runs` | `evalRuns.list` | List eval runs. |
| `GET` | `/backend/v3/api/llm/eval_runs/{evalRunId}` | `evalRuns.retrieve` | Retrieve eval run result. |
| `GET` | `/backend/v3/api/llm/retrieval_traces` | `retrievalTraces.list` | List retrieval traces. |
| `GET` | `/backend/v3/api/llm/retrieval_traces/{traceId}` | `retrievalTraces.retrieve` | Retrieve retrieval trace. |
| `GET` | `/backend/v3/api/llm/audit_logs` | `auditLogs.list` | List memory audit logs. |
| `POST` | `/backend/v3/api/llm/retention_jobs` | `retentionJobs.create` | Run retention/deletion propagation job. |
| `POST` | `/backend/v3/api/llm/migration_jobs` | `migrationJobs.create` | Start implementation/profile migration. |
| `GET` | `/backend/v3/api/llm/migration_jobs/{migrationJobId}` | `migrationJobs.retrieve` | Retrieve migration job. |
...
```text
services/sdkwork-llm-storage-sqlx/migrations/postgres/V202606100001__memory_core.sql
services/sdkwork-llm-storage-sqlx/migrations/sqlite/V202606100001__memory_core.sql
```
...
```text
uk_llm_space_uuid (tenant_id, uuid)
uk_llm_space_owner_type (tenant_id, owner_subject_type, owner_subject_id, space_type)
idx_llm_space_tenant_status (tenant_id, status, updated_at)
```
...
```text
uk_llm_event_uuid (tenant_id, uuid)
idx_llm_event_space_time (tenant_id, space_id, event_time, id)
idx_llm_event_session_time (tenant_id, session_id, event_time)
idx_llm_event_type_time (tenant_id, event_type, event_time)
idx_llm_event_hash (tenant_id, payload_hash)
```
...
```text
uk_llm_record_uuid (tenant_id, uuid)
idx_llm_record_scope_type_status (tenant_id, space_id, scope, memory_type, status, updated_at)
idx_llm_record_user_type (tenant_id, user_id, memory_type, status, updated_at)
idx_llm_record_subject_predicate (tenant_id, space_id, subject, predicate, status)
idx_llm_record_validity (tenant_id, valid_from, valid_to, expires_at)
idx_llm_record_supersession (tenant_id, supersedes_memory_id, superseded_by_memory_id)
```
...
```text
uk_llm_record_source_uuid (tenant_id, uuid)
uk_llm_record_source_pair (tenant_id, memory_id, event_id, source_role)
idx_llm_record_source_event (tenant_id, event_id)
```
...
```text
uk_llm_entity_uuid (tenant_id, uuid)
uk_llm_entity_name (tenant_id, space_id, entity_type, canonical_name)
idx_llm_entity_type_status (tenant_id, space_id, entity_type, status)
```
...
```text
uk_llm_edge_uuid (tenant_id, uuid)
idx_llm_edge_source (tenant_id, space_id, source_entity_id, relation_type, status)
idx_llm_edge_target (tenant_id, space_id, target_entity_id, relation_type, status)
idx_llm_edge_validity (tenant_id, valid_from, valid_to)
```
...
```text
uk_llm_candidate_uuid (tenant_id, uuid)
idx_llm_candidate_state (tenant_id, space_id, decision_state, updated_at)
idx_llm_candidate_target (tenant_id, target_memory_id)
```
...
```text
uk_llm_habit_uuid (tenant_id, uuid)
uk_llm_habit_key (tenant_id, space_id, user_id, habit_key)
idx_llm_habit_stage (tenant_id, space_id, stage, confidence, updated_at)
```
...
```text
uk_llm_habit_signal_uuid (tenant_id, uuid)
idx_llm_habit_signal_habit (tenant_id, habit_id, observed_at)
idx_llm_habit_signal_event (tenant_id, event_id)
```
...
```text
uk_llm_learning_job_uuid (tenant_id, uuid)
uk_llm_learning_job_idempotency (tenant_id, job_type, idempotency_key)
idx_llm_learning_job_state (tenant_id, job_type, state, priority, created_at)
```
...
```text
uk_llm_index_uuid (tenant_id, uuid)
uk_llm_index_kind_space (tenant_id, space_id, index_kind, schema_version)
idx_llm_index_status (tenant_id, space_id, index_kind, status)
```
...
```text
uk_llm_index_entry_uuid (tenant_id, uuid)
uk_llm_index_entry_memory (tenant_id, index_id, memory_id, entry_kind)
idx_llm_index_entry_hash (tenant_id, index_id, entry_hash)
```
...
```text
uk_llm_retrieval_profile_uuid (tenant_id, uuid)
idx_llm_retrieval_profile_scope (tenant_id, space_id, status, updated_at)
```
...
```text
uk_llm_retrieval_trace_uuid (tenant_id, uuid)
idx_llm_retrieval_trace_profile_created (tenant_id, retrieval_profile_id, created_at)
idx_llm_retrieval_trace_actor_created (tenant_id, actor_id, created_at)
```
...
```text
uk_llm_retrieval_hit_uuid (tenant_id, uuid)
idx_llm_retrieval_hit_trace_rank (tenant_id, retrieval_trace_id, result_rank)
idx_llm_retrieval_hit_memory (tenant_id, memory_id, status)
```
...
```text
uk_llm_context_pack_uuid (tenant_id, uuid)
idx_llm_context_pack_trace (tenant_id, retrieval_trace_id)
idx_llm_context_pack_actor_created (tenant_id, actor_id, created_at)
```
...
```text
uk_llm_implementation_profile_uuid (tenant_id, uuid)
idx_llm_implementation_profile_kind (tenant_id, implementation_kind, status)
```
...
```text
uk_llm_provider_binding_uuid (tenant_id, uuid)
uk_llm_provider_binding_code (tenant_id, provider_kind, provider_code)
idx_llm_provider_binding_health (tenant_id, provider_kind, health_state, updated_at)
```
...
```text
uk_llm_policy_uuid (tenant_id, uuid)
idx_llm_policy_type_scope (tenant_id, policy_type, scope, status)
```
...
```text
uk_llm_audit_log_uuid (tenant_id, uuid)
idx_llm_audit_actor_time (tenant_id, actor_type, actor_id, created_at)
idx_llm_audit_resource_time (tenant_id, resource_type, resource_id, created_at)
idx_llm_audit_action_time (tenant_id, action, created_at)
```
...
```text
uk_llm_eval_run_uuid (tenant_id, uuid)
idx_llm_eval_run_type_state (tenant_id, eval_type, state, created_at)
```
...
```text
uk_llm_outbox_event_uuid (tenant_id, uuid)
idx_llm_outbox_state (tenant_id, publish_state, created_at)
```
...
1. Open API paths start with `/llm/v3/api` and do not use `/app/v3/api` or `/backend/v3/api`.
...
7. Every generated operation declares `x-sdkwork-owner: sdkwork-llm`.
8. Open operations declare `x-sdkwork-api-authority: sdkwork-llm-open-api`.
9. App operations declare `x-sdkwork-api-authority: sdkwork-llm.app`.
10. Backend operations declare `x-sdkwork-api-authority: sdkwork-llm.backend`.
...
sdks/sdkwork-llm-sdk skeleton
sdks/sdkwork-llm-app-sdk skeleton
sdks/sdkwork-llm-backend-sdk skeleton
...
llm_space, llm_event, llm_record, llm_record_source, llm_candidate, llm_habit
SQL/keyword/dictionary/time/event retrievers
...
6. Whether graph memory starts with relational `llm_entity`/`llm_edge` tables or an external graph provider behind the same port.
