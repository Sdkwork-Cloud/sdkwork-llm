> Migrated from `docs/superpowers/specs/2026-06-10-commercial-llm-management-design.md` on 2026-06-24.
...
This document extends SDKWork LLM from a Phase 1 contract/runtime skeleton into a commercial memory management platform design.
...
Root SDKWork standards remain authoritative. This document narrows them for SDKWork LLM.

## 2. Current State

The current repository already has a strong Phase 1 base:

- `llm_space` models memory namespaces with `owner_subject_type` and `owner_subject_id`.
- `llm_record` models canonical memory facts and supports scope, user, subject, predicate, object, status, and supersession.
- `llm_entity` and `llm_edge` model graph-compatible entities and relationships.
- `llm_retrieval_profile`, `llm_retrieval_trace`, `llm_retrieval_hit`, and `llm_context_pack` model retrieval and context assembly.
- `llm_implementation_profile`, `llm_provider_binding`, and `llm_policy` model implementation and provider governance.
...
- `LlmSpace.ownerSubjectType/ownerSubjectId` can identify an owner but does not express all attachment roles.
- `LlmRecord.scope/userId/subject/predicate/objectText` can model facts but is not a first-class binding contract.
- `LlmRetrievalRequest.filters` can pass flexible constraints but is not an auditable relationship contract.
- `llm_entity` and `llm_edge` exist in schema registry but are not first-class API resources.
...
SDKWork LLM is commercially ready only when a tenant can answer these questions through API and SDK calls:
...
`llm_record` remains the canonical durable memory fact. Attachment and capability records never replace canonical memory state.
...
Exposes `llm_entity` as a first-class API resource.
...
Exposes `llm_edge` as a first-class API resource.
...
Attaches a `llm_policy` row to a subject, space, entity, binding, capability binding, or implementation profile.
...
```text
GET    /llm/v3/api/llm/subjects                    subjects.list
POST   /llm/v3/api/llm/subjects                    subjects.create
GET    /llm/v3/api/llm/subjects/{subjectId}        subjects.retrieve
PATCH  /llm/v3/api/llm/subjects/{subjectId}        subjects.update

GET    /llm/v3/api/llm/entities                    entities.list
POST   /llm/v3/api/llm/entities                    entities.create
GET    /llm/v3/api/llm/entities/{entityId}         entities.retrieve
PATCH  /llm/v3/api/llm/entities/{entityId}         entities.update

GET    /llm/v3/api/llm/edges                       edges.list
POST   /llm/v3/api/llm/edges                       edges.create
GET    /llm/v3/api/llm/edges/{edgeId}              edges.retrieve
PATCH  /llm/v3/api/llm/edges/{edgeId}              edges.update
DELETE /llm/v3/api/llm/edges/{edgeId}              edges.delete

GET    /llm/v3/api/llm/bindings                    bindings.list
POST   /llm/v3/api/llm/bindings                    bindings.create
GET    /llm/v3/api/llm/bindings/{bindingId}        bindings.retrieve
PATCH  /llm/v3/api/llm/bindings/{bindingId}        bindings.update
DELETE /llm/v3/api/llm/bindings/{bindingId}        bindings.delete
POST   /llm/v3/api/llm/bindings/resolve            bindings.resolve

POST   /llm/v3/api/llm/capabilities/resolve        capabilities.resolve
```
...
```text
GET    /app/v3/api/llm/subjects                    subjects.list
POST   /app/v3/api/llm/subjects                    subjects.create
GET    /app/v3/api/llm/subjects/{subjectId}        subjects.retrieve
PATCH  /app/v3/api/llm/subjects/{subjectId}        subjects.update
GET    /app/v3/api/llm/subjects/{subjectId}/records       subjects.records.list
GET    /app/v3/api/llm/subjects/{subjectId}/bindings       subjects.bindings.list
POST   /app/v3/api/llm/subjects/{subjectId}/context_packs  subjects.contextPacks.create
POST   /app/v3/api/llm/subjects/{subjectId}/learning/enable   subjects.learning.enable
POST   /app/v3/api/llm/subjects/{subjectId}/learning/disable  subjects.learning.disable

GET    /app/v3/api/llm/entities                    entities.list
POST   /app/v3/api/llm/entities                    entities.create
GET    /app/v3/api/llm/entities/{entityId}         entities.retrieve
PATCH  /app/v3/api/llm/entities/{entityId}         entities.update

GET    /app/v3/api/llm/bindings                    bindings.list
POST   /app/v3/api/llm/bindings                    bindings.create
GET    /app/v3/api/llm/bindings/{bindingId}        bindings.retrieve
PATCH  /app/v3/api/llm/bindings/{bindingId}        bindings.update
DELETE /app/v3/api/llm/bindings/{bindingId}        bindings.delete
POST   /app/v3/api/llm/bindings/resolve            bindings.resolve

GET    /app/v3/api/llm/policy_assignments          policyAssignments.list
POST   /app/v3/api/llm/policy_assignments          policyAssignments.create
PATCH  /app/v3/api/llm/policy_assignments/{policyAssignmentId} policyAssignments.update
```
...
```text
GET    /backend/v3/api/llm/subjects                subjects.list
POST   /backend/v3/api/llm/subjects                subjects.create
GET    /backend/v3/api/llm/subjects/{subjectId}    subjects.retrieve
PATCH  /backend/v3/api/llm/subjects/{subjectId}    subjects.update
GET    /backend/v3/api/llm/subjects/{subjectId}/effective_capabilities subjects.effectiveCapabilities.retrieve
GET    /backend/v3/api/llm/subjects/{subjectId}/effective_policies      subjects.effectivePolicies.retrieve

GET    /backend/v3/api/llm/entities                entities.list
POST   /backend/v3/api/llm/entities                entities.create
GET    /backend/v3/api/llm/entities/{entityId}     entities.retrieve
PATCH  /backend/v3/api/llm/entities/{entityId}     entities.update
POST   /backend/v3/api/llm/entities/{entityId}/merge entities.merge

GET    /backend/v3/api/llm/edges                   edges.list
POST   /backend/v3/api/llm/edges                   edges.create
GET    /backend/v3/api/llm/edges/{edgeId}          edges.retrieve
PATCH  /backend/v3/api/llm/edges/{edgeId}          edges.update
DELETE /backend/v3/api/llm/edges/{edgeId}          edges.delete

GET    /backend/v3/api/llm/bindings                bindings.list
POST   /backend/v3/api/llm/bindings                bindings.create
GET    /backend/v3/api/llm/bindings/{bindingId}    bindings.retrieve
PATCH  /backend/v3/api/llm/bindings/{bindingId}    bindings.update
DELETE /backend/v3/api/llm/bindings/{bindingId}    bindings.delete
POST   /backend/v3/api/llm/bindings/bulk_upsert    bindings.bulkUpsert
POST   /backend/v3/api/llm/bindings/bulk_delete    bindings.bulkDelete
POST   /backend/v3/api/llm/bindings/resolve        bindings.resolve

GET    /backend/v3/api/llm/capability_bindings     capabilityBindings.list
POST   /backend/v3/api/llm/capability_bindings     capabilityBindings.create
GET    /backend/v3/api/llm/capability_bindings/{capabilityBindingId} capabilityBindings.retrieve
PATCH  /backend/v3/api/llm/capability_bindings/{capabilityBindingId} capabilityBindings.update
DELETE /backend/v3/api/llm/capability_bindings/{capabilityBindingId} capabilityBindings.delete

GET    /backend/v3/api/llm/policy_assignments      policyAssignments.list
POST   /backend/v3/api/llm/policy_assignments      policyAssignments.create
GET    /backend/v3/api/llm/policy_assignments/{policyAssignmentId} policyAssignments.retrieve
PATCH  /backend/v3/api/llm/policy_assignments/{policyAssignmentId} policyAssignments.update
DELETE /backend/v3/api/llm/policy_assignments/{policyAssignmentId} policyAssignments.delete

POST   /backend/v3/api/llm/relation_rebuild_jobs   relationRebuildJobs.create
GET    /backend/v3/api/llm/relation_rebuild_jobs/{jobId} relationRebuildJobs.retrieve

GET    /backend/v3/api/llm/commercial_readiness    commercialReadiness.retrieve
POST   /backend/v3/api/llm/commercial_readiness/rebuild commercialReadiness.rebuild
```
...
```text
uk_llm_subject_uuid (tenant_id, uuid)
uk_llm_subject_ref (tenant_id, subject_type, subject_ref)
idx_llm_subject_status (tenant_id, subject_type, status, updated_at)
idx_llm_subject_space (tenant_id, default_space_id, status)
```
...
```text
uk_llm_memory_binding_uuid (tenant_id, uuid)
idx_llm_binding_source_subject (tenant_id, source_subject_id, binding_kind, status)
idx_llm_binding_source_entity (tenant_id, source_entity_id, binding_kind, status)
idx_llm_binding_target_memory (tenant_id, target_memory_id, binding_kind, status)
idx_llm_binding_target_space (tenant_id, target_space_id, binding_kind, status)
idx_llm_binding_external_source (tenant_id, source_external_ref_source, source_external_ref_type, source_external_ref_id)
idx_llm_binding_validity (tenant_id, valid_from, valid_to, status)
```
...
```text
uk_llm_capability_binding_uuid (tenant_id, uuid)
idx_llm_capability_target (tenant_id, target_type, target_id, capability_code, status)
idx_llm_capability_priority (tenant_id, capability_code, mode, priority)
idx_llm_capability_validity (tenant_id, valid_from, valid_to, status)
```

### 7.4 `llm_policy_assignment`

System of record for attaching `llm_policy` to a target.
...
```text
uk_llm_policy_assignment_uuid (tenant_id, uuid)
idx_llm_policy_assignment_target (tenant_id, target_type, target_id, status, priority)
idx_llm_policy_assignment_policy (tenant_id, policy_id, status)
idx_llm_policy_assignment_validity (tenant_id, valid_from, valid_to, status)
```
...
- Explicit remember requests may create both `llm_record` and `mem_memory_binding`.
...
```text
llm.open.subjects.read
llm.open.subjects.write
llm.open.entities.read
llm.open.entities.write
llm.open.bindings.read
llm.open.bindings.write
llm.open.capabilities.resolve
