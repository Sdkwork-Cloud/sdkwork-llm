> Migrated from `docs/superpowers/plans/2026-06-10-Llm-spi-plugin-runtime-implementation-plan.md` on 2026-06-24.
...
**Goal:** Build the first executable SDKWork LLM SPI/plugin runtime layer so implementation profiles can compose provider-neutral ports without changing public Open API, App API, or Backend API contracts.

**Architecture:** Start with static Rust plugin registration for 0.1.0. Keep `sdkwork-llm-spi` framework-neutral and provider-neutral, add a small `sdkwork-llm-runtime` resolver/registry, and create the first `sdkwork-llm-plugin-native-sql` manifest package as the no-embedding baseline. Dynamic loading, backend plugin-list APIs, and provider-specific bridges are deferred until native SQL conformance is real.
...
- Existing MVP plan: `docs/superpowers/plans/2026-06-10-llm-open-api-and-no-embedding-mvp.md`
...
- Create: `Cargo.toml`
- Create: `crates/sdkwork-llm-spi/Cargo.toml`
- Create: `crates/sdkwork-llm-spi/src/lib.rs`
- Create: `crates/sdkwork-llm-spi/src/error.rs`
- Create: `crates/sdkwork-llm-spi/src/manifest.rs`
- Create: `crates/sdkwork-llm-spi/src/ports.rs`
- Create: `crates/sdkwork-llm-spi/src/registry.rs`
- Create: `crates/sdkwork-llm-spi/src/runtime.rs`
- Create: `crates/sdkwork-llm-spi/tests/manifest_contract.rs`
- Create: `crates/sdkwork-llm-spi/tests/registry_contract.rs`
- Create: `crates/sdkwork-llm-runtime/Cargo.toml`
- Create: `crates/sdkwork-llm-runtime/src/lib.rs`
- Create: `crates/sdkwork-llm-runtime/src/profile.rs`
- Create: `crates/sdkwork-llm-runtime/tests/profile_resolution_contract.rs`
- Create: `crates/sdkwork-llm-test-support/Cargo.toml`
- Create: `crates/sdkwork-llm-test-support/src/lib.rs`
- Create: `plugins/sdkwork-llm-plugin-native-sql/Cargo.toml`
- Create: `plugins/sdkwork-llm-plugin-native-sql/sdkwork.llm.plugin.json`
- Create: `plugins/sdkwork-llm-plugin-native-sql/src/lib.rs`
- Create: `plugins/sdkwork-llm-plugin-native-sql/src/manifest.rs`
- Create: `plugins/sdkwork-llm-plugin-native-sql/tests/manifest_matches_json.rs`
...
conformance stored in llm_eval_run initially
...
Create root `Cargo.toml` with `sdkwork-llm-spi`, `sdkwork-llm-runtime`, `sdkwork-llm-test-support`, and `plugins/sdkwork-llm-plugin-native-sql` as members. Use workspace dependencies for `async-trait`, `serde`, `serde_json`, and `thiserror`.

- [ ] **Step 3: Add minimal `lib.rs` exports only**

`crates/sdkwork-llm-spi/src/lib.rs` must only declare modules and re-export stable public SPI types.
...
- Create: `crates/sdkwork-llm-spi/src/manifest.rs`
- Create: `crates/sdkwork-llm-spi/src/error.rs`
- Test: `crates/sdkwork-llm-spi/tests/manifest_contract.rs`
...
```rust
use sdkwork_llm_spi::{
    LlmImplementationKind, LlmPluginManifest, MemoryPluginRole,
};

#[test]
fn native_sql_manifest_deserializes_and_declares_no_embedding_baseline() {
    let manifest: LlmPluginManifest = serde_json::from_str(r#"{
      "schemaVersion": 1,
      ...
      "portExports": [{"port": "LlmRecordStorePort", "builder": "build_native_sql_record_store"}],
      "providerKinds": [],
    ...
    assert!(manifest.implementation_kinds.contains(&LlmImplementationKind::NativeSql));
    assert!(manifest.implementation_kinds.contains(&LlmImplementationKind::LocalEmbedded));
...
```powershell
cargo test -p sdkwork-llm-spi --test manifest_contract
```
...
kind != sdkwork.llm.plugin
schemaVersion != 1
pluginId not starting with sdkwork-llm-plugin-
...
**Files:**
- Create: `crates/sdkwork-llm-spi/src/ports.rs`
- Test: `crates/sdkwork-llm-spi/tests/ports_contract.rs`
...
```powershell
cargo test -p sdkwork-llm-spi --test ports_contract
```
...
**Files:**
- Create: `crates/sdkwork-llm-spi/src/registry.rs`
- Test: `crates/sdkwork-llm-spi/tests/registry_contract.rs`
...
lookup required port export LlmRecordStorePort
...
```powershell
cargo test -p sdkwork-llm-spi --test registry_contract
```
...
- Create: `crates/sdkwork-llm-runtime/src/lib.rs`
- Create: `crates/sdkwork-llm-runtime/src/profile.rs`
- Test: `crates/sdkwork-llm-runtime/tests/profile_resolution_contract.rs`
...
```powershell
cargo test -p sdkwork-llm-runtime --test profile_resolution_contract
```
...
- Create: `plugins/sdkwork-llm-plugin-native-sql/src/lib.rs`
- Create: `plugins/sdkwork-llm-plugin-native-sql/src/manifest.rs`
- Test: `plugins/sdkwork-llm-plugin-native-sql/tests/manifest_matches_json.rs`
...
```text
JSON manifest deserializes into LlmPluginManifest
...
```powershell
cargo test -p sdkwork-llm-plugin-native-sql --test manifest_matches_json
```
...
- Create: `crates/sdkwork-llm-test-support/Cargo.toml`
- Create: `crates/sdkwork-llm-test-support/src/lib.rs`
- Test: `crates/sdkwork-llm-test-support/tests/conformance_harness_contract.rs`
...
```powershell
cargo test -p sdkwork-llm-test-support --test conformance_harness_contract
```
...
cargo test -p sdkwork-llm-plugin-native-sql
cargo test -p sdkwork-llm-test-support
