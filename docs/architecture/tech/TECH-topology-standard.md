...
`application-http-gateway`: Memory exposes open, app, and backend HTTP surfaces through `sdkwork-router-memory-*` route crates. Phase 1 runs all surfaces in a unified `sdkwork-llm-api-server` process.
