> Migrated from `docs/superpowers/specs/2026-06-10-llm-spi-plugin-architecture-design.md` on 2026-06-24.
> Owner: SDKWork maintainers

## 1. Purpose

This document upgrades SDKWork LLM from a provider-switchable contract skeleton into a runtime architecture that can support multiple implementations without changing public Open API, App API, or Backend API contracts.
...
- `llm_index`, `llm_index_entry`, `llm_retrieval_profile`, `llm_retrieval_trace`, and `llm_context_pack` represent derived retrieval and context state.
- `llm_implementation_profile`, `llm_provider_binding`, and `llm_policy` represent runtime selection and governance.
...
SDKWork LLM should not clone one of these systems. It should provide a stable platform core that can host these patterns as interchangeable plugins and evaluate them through the same contract suite.
...
The implementation consequence is explicit: SDKWork LLM must not hard-code one memory engine. The core owns governed memory state transitions. Plugin ports host the interchangeable storage, retrieval, graph, vector, archival, background-learning, and external bridge patterns.

## 4. Design Principles

1. Canonical truth is stable.
   `llm_record`, `llm_event`, source links, audit logs, and deletion state are the durable authority. Derived indexes may be rebuilt or switched.
...
5. Profiles configure composition.
   `llm_implementation_profile`, `llm_provider_binding`, and typed runtime config select plugins and provider bindings. Code forks must not select behavior implicitly.
...
- Resolve `LlmScopeContext` from typed appbase request context.
...
typed runtime config
  ...
  -> LlmCoreRuntime builder
  -> port conformance preflight
  -> health and readiness
  -> request handlers call product services
```

`LlmCoreRuntime` is the stable composition object for one process/runtime profile. It wires the core services to plugin-provided ports.

```ts
export interface LlmRuntimePlugin {
...
export interface LlmCoreRuntime {
...
```ts
export interface LlmRecordStorePort {
  create(command: CreateLlmRecordCommand): Promise<LlmRecord>;
  retrieve(query: RetrieveLlmRecordQuery): Promise<LlmRecord | null>;
  list(query: ListLlmRecordsQuery): Promise<Page<LlmRecord>>;
  update(command: UpdateLlmRecordCommand): Promise<LlmRecord>;
  supersede(command: SupersedeLlmRecordCommand): Promise<LlmRecord>;
  markDeleted(command: DeleteLlmRecordCommand): Promise<LlmDeletionReceipt>;
}

export interface LlmEventStorePort {
  append(command: AppendLlmEventCommand): Promise<LlmEvent>;
  retrieve(query: RetrieveLlmEventQuery): Promise<LlmEvent | null>;
  list(query: ListLlmEventsQuery): Promise<Page<LlmEvent>>;
}

export interface LlmAuditStorePort {
  append(command: AppendLlmAuditCommand): Promise<LlmAuditRecord>;
  list(query: ListMemoryAuditQuery): Promise<Page<LlmAuditRecord>>;
}

export interface LlmOutboxStorePort {
  append(command: AppendLlmOutboxCommand): Promise<LlmOutboxEvent>;
  retrieve(query: RetrieveLlmOutboxQuery): Promise<LlmOutboxEvent | null>;
  listPending(query: ListPendingLlmOutboxQuery): Promise<Page<LlmOutboxEvent>>;
  markPublished(command: MarkLlmOutboxPublishedCommand): Promise<LlmOutboxEvent>;
  markFailed(command: MarkLlmOutboxFailedCommand): Promise<LlmOutboxEvent>;
}
...
export interface MemoryExtractionPort {
  extract(command: ExtractLlmCandidatesCommand): Promise<LlmCandidateDraft[]>;
}

export interface LlmCandidateStorePort {
  create(command: CreateLlmCandidateCommand): Promise<LlmCandidate>;
  decide(command: DecideLlmCandidateCommand): Promise<MemoryDecisionResult>;
  list(query: ListLlmCandidatesQuery): Promise<Page<LlmCandidate>>;
}

export interface LlmHabitLearnerPort {
  observe(command: ObserveHabitSignalCommand): Promise<LlmHabitSignalResult>;
  score(command: ScoreHabitCommand): Promise<LlmHabitScore>;
  promote(command: PromoteHabitCommand): Promise<LlmHabitPromotionResult>;
  decay(command: DecayHabitCommand): Promise<LlmHabitDecayResult>;
}
```
...
```ts
export interface LlmRetrieverPort {
  retrieverCode(): string;
  retrieverKind(): LlmRetrieverKind;
  capabilities(): MemoryRetrieverCapabilities;
  retrieve(command: RetrieveLlmCandidatesCommand): Promise<LlmRetrieverResult>;
}

export type LlmRetrieverKind =
  | "sql"
  | "keyword"
  | "dictionary"
  | "time"
...
2. Select retrievers from `llm_retrieval_profile`.
...
```ts
export interface LlmIndexPort {
  indexKind(): MemoryIndexKind;
  index(command: IndexMemoryCommand): Promise<LlmIndexReceipt>;
  remove(command: RemoveMemoryIndexCommand): Promise<LlmIndexReceipt>;
...
export interface ExternalLlmBridgePort {
  providerCode(): string;
  import(command: ExternalLlmImportCommand): Promise<ExternalLlmImportResult>;
  export(command: ExternalLlmExportCommand): Promise<ExternalLlmExportResult>;
  delete(command: ExternalLlmDeleteCommand): Promise<ExternalLlmDeletionReceipt>;
  shadowRead(command: ExternalLlmShadowReadCommand): Promise<ExternalLlmShadowReadResult>;
}
```
...
```ts
export interface LlmContextAssemblerPort {
  assemble(command: AssembleLlmContextCommand): Promise<LlmContextPackDraft>;
}

export interface LlmEvaluationPort {
  run(command: RunLlmEvalCommand): Promise<LlmEvalRunResult>;
}
```
...
Every runtime plugin declares a `LlmPluginManifest`. This manifest is source-controlled with the plugin package and can also be exposed in backend-admin health/capability summaries.

```ts
export interface LlmPluginManifest {
  schemaVersion: 1;
  ...
  version: string;
  owner: string;
  implementationKinds: LlmImplementationKind[];
  ...
  retrieverKinds: LlmRetrieverKind[];
...
- `pluginId` uses lowercase kebab-case and starts with `sdkwork-llm-plugin-` for SDKWork-owned plugins.
- `kind` is exactly `sdkwork.llm.plugin`.
...
```json
{
  ...
  "observability": {
    "metricsPrefix": "sdkwork_llm_native_sql",
    "redactsPayloads": true
  },
  "conformance": {
    "suite": "sdkwork-llm-plugin-conformance",
    "suiteVersion": "0.1.0"
  }
}
```
...
- Must prove projection rebuild from `llm_event`.
- May use `llm_record` as projection or canonical snapshot, but event evidence remains required.
...
- Relational `llm_entity` and `llm_edge` are the first portable graph model.
...
Runtime plugins are not Codex agent plugins. Do not place runtime LLM plugins under `.sdkwork/plugins/`.
...
plugins/
  sdkwork-llm-plugin-native-sql/
    Cargo.toml
    sdkwork.llm.plugin.json
    src/lib.rs
    src/manifest.rs
    src/stores.rs
    src/retrievers/
    tests/conformance_native_sql.rs
  sdkwork-llm-plugin-local-embedded/
  sdkwork-llm-plugin-search-opensearch/
  sdkwork-llm-plugin-graph-relational/
  sdkwork-llm-plugin-external-bridge/
services/
  sdkwork-llm-product/
  sdkwork-llm-service/
packages/native-rust/routes/
  open-api/sdkwork-routes-llm-open-api/
  app-api/sdkwork-routes-llm-app-api/
  backend-api/sdkwork-routes-llm-backend-api/
```
...
pub fn register_builtin_memory_plugins(registry: &mut MemoryPluginRegistry) {
    sdkwork_llm_plugin_native_sql::register(registry);
}
```
...
- `llm_implementation_profile` selects the active implementation kind, role, status, capabilities, rollout, and plugin config references.
- `llm_provider_binding` selects provider kind, provider code, endpoint ref, secret ref, model ref, capabilities, health, and safe non-secret config.
- `llm_retrieval_profile` selects retriever list, fusion policy, rerank policy, top K, token budget, and degraded-mode behavior.
- `llm_policy` selects retention, privacy, review, learning, retrieval, provider, export, and deletion behavior.
...
- Degraded retrieval must be recorded in `llm_retrieval_trace`.
...
```powershell
cargo test -p sdkwork-llm-spi
cargo test -p sdkwork-llm-runtime
cargo test -p sdkwork-llm-plugin-native-sql --test conformance_native_sql
...
- `GET /backend/v3/api/llm/plugins`
- `GET /backend/v3/api/llm/plugins/{pluginId}`
- `POST /backend/v3/api/llm/plugins/{pluginId}/validate`
- `POST /backend/v3/api/llm/plugins/{pluginId}/conformance_runs`
...
- `crates/sdkwork-llm-spi`
- `crates/sdkwork-llm-runtime`
...
- `plugins/sdkwork-llm-plugin-native-sql`
...
2. `local_embedded` starts as a capability/profile inside `sdkwork-llm-plugin-native-sql`.
   A separate `sdkwork-llm-plugin-local-embedded` package may be split later only when desktop/local filesystem ownership, package release, and runtime directory policy require it.

3. Plugin manifests use JSON manifest plus Rust constant.
   Each runtime plugin keeps a source-controlled `sdkwork.llm.plugin.json` for tooling and exposes the same manifest through a Rust constant/function for static registration tests. The Rust constant must be generated from or verified against the JSON manifest before release.
...
5. Plugin conformance results are stored in `llm_eval_run` for 0.1.0.
...
9. Static scans can distinguish runtime LLM plugins under `plugins/` from agent workspace plugins under `.sdkwork/plugins/`.
