...
foreach ($specPath in $specPaths) {
    ...
    if ($spec.'x-sdkwork-owner' -ne "sdkwork-llm") {
        throw "OpenAPI spec must declare x-sdkwork-owner=sdkwork-llm: $specPath"
    }
...
Write-Host "Verified LLM OpenAPI operationIds and schemas across $($specPaths.Count) specs."
