...
const baselinePath = path.join(
  root,
  'database/ddl/baseline/postgres/0001_llm_legacy_baseline.sql',
);
...
const prefixRegistry = {
  ...
  prefixes: [{ prefix: 'llm_', owner: 'llm-platform', domain: 'llm' }],
};
...
manifest.tablePrefix = 'llm_';
manifest.lifecycle.autoMigrate = true;
fs.writeFileSync(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`);

process.stdout.write(`materialized ${tableNames.length} tables into llm database contract\n`);
